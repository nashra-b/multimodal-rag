...
class ImageSummarizer:
    ...
    def summarize_batch(self, image_elements: list, context: str = "") -> list:
        results = []
        for el in image_elements:
            image_path = el.get("image_path") if isinstance(el, dict) else getattr(el, "image_path", None)
            if not image_path:
                results.append(el)
                continue
            summary = self.summarize(image_path, context)
